# ═══════════════════════════════════════════════════════
# main.tf – Lab 04 with Terraform
# Deploys everything from the original lab
# ═══════════════════════════════════════════════════════
...
# ── ManufacturingVnet ──────────────────────────────────
resource "azurerm_virtual_network" "manuf" {
...
resource "azurerm_subnet" "sensor1" {
  name                 = "SensorSubnet1"
  resource_group_name  = azurerm_resource_group.rg.name
  virtual_network_name = azurerm_virtual_network.manuf.name
...
resource "azurerm_dns_zone" "public" {
  name                = "lab04-contoso-${random_id.dns_suffix.hex}.com"
  resource_group_name = azurerm_resource_group.rg.name
}
...
resource "azurerm_private_dns_zone_virtual_network_link" "link" {
  ...
  virtual_network_id    = azurerm_virtual_network.manuf.id
}
